...
public class TunnelConnectionManagerFactory {
  ...
  private static int getLocalPort() {
    try (ServerSocket socket = new ServerSocket(FIRST_AVAILABLE_PORT)) {
      ...
      throw new CircusTrainException("Unable to bind to a free localhost port", e);
    }
  }
  ...
  public TunnelConnectionManager create(
    ...
    try {
      ...
      throw new CircusTrainException("Unable to create a TunnelConnectionManager: " + tunnelExpression, e);
    } finally {
