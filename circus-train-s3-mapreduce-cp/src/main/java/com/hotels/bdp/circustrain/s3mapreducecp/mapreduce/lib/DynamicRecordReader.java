...
/**
 * The DynamicRecordReader is used in conjunction with the DynamicInputFormat to implement the "Worker pattern" for
 * S3MapReduceCp. The DynamicRecordReader is responsible for: 1. Presenting the contents of each chunk to
 * S3MapReduceCp's mapper. 2. Acquiring a new chunk when the current chunk has been completely consumed, transparently.
 */
public class DynamicRecordReader<K, V> extends RecordReader<K, V> {
  ...
  /**
   * Implementation of RecordReader::nextValue(). Reads the contents of the current chunk and returns them. When a chunk
   * has been completely exhausted, a new chunk is acquired and read, transparently.
   *
  ...
  public boolean nextKeyValue() throws IOException, InterruptedException {
    ...
    if (chunk == null) {
      LOG.debug("{}: RecordReader is null. No records to be read.", taskId);
      return false;
    }
    ...
    LOG.debug("{}: Current chunk exhausted. Attempting to pick up new one.", taskId);
  ...
  private int getNumChunksLeft() throws IOException {
    ...
    if (tooLongSinceLastDirScan
        || (!isChunkDirAlreadyScanned && numRecordsProcessedByThisMap % numRecordsPerChunk > numRecordsPerChunk / 2)) {
