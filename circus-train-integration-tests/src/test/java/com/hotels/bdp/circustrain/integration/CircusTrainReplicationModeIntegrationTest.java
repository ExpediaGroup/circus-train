...
public class CircusTrainReplicationModeIntegrationTest {
  ...
  public void unpartitionedTableFullOverwrite() throws Exception {
    ...
    String eventId = "previous_event";
    final URI replicaLocation = toUri("s3a://replica/", DATABASE, UNPARTITIONED_TABLE + "/" + eventId);
    ...
    // Adding file to replica bucket - expect this to be removed in the replication
    final File replicaDataFile = temporaryFolder.newFile();
    FileUtils.writeStringToFile(replicaDataFile, "1\trob\tbristol\t23\tred\n2\tsam\ttoronto\t27\tblue\n");
    String replicaFileKey = String.format("%s/%s/%s/%s", DATABASE, UNPARTITIONED_TABLE, eventId, PART_00000);
    s3Client.putObject("replica", replicaFileKey, replicaDataFile);
    ...
    exit.checkAssertionAfterwards(new Assertion() {
      ...
      public void checkAssertion() throws Exception {
        ...
        // Assert files copied from source and replica file is deleted
  ...
  public void partitionedTableFullOverwrite() throws Exception {
    final URI sourceTableLocation = toUri("s3a://source/", DATABASE, SOURCE_MANAGED_PARTITIONED_TABLE);
    TestUtils.createPartitionedTable(sourceCatalog.client(), DATABASE, SOURCE_MANAGED_PARTITIONED_TABLE, sourceTableLocation);
    // adjusting the sourceTable, mimicking the change we want to update
    Table sourceTable = sourceCatalog.client().getTable(DATABASE, SOURCE_MANAGED_PARTITIONED_TABLE);
    sourceTable.putToParameters("paramToUpdate", "updated");
    sourceCatalog.client().alter_table(sourceTable.getDbName(), sourceTable.getTableName(), sourceTable);

    addPartitionsToSource(sourceTable, sourceTableLocation);

    // create source table with extra columns and an additional partition that is expected to be removed.
  ...
  private void addPartitionsToReplica(Table replicaTable, URI replicaLocation) throws Exception {
    final File replicaDataFile = temporaryFolder.newFile();
    FileUtils.writeStringToFile(replicaDataFile, "1\trob\tbristol\t23\tred\n2\tsam\ttoronto\t27\tblue\n");
    String partitionAmerica = "/dummyEventID/continent=America";
    String partitionMexico = partitionAmerica + "/country=Mexico";
    String partitionFileLocation = TARGET_PARTITIONED_TABLE + partitionMexico;
    String replicaFileKey = String.format("%s/%s/%s", DATABASE, partitionFileLocation, PART_00000);
    s3Client.putObject("replica", replicaFileKey, replicaDataFile);

    URI partitionURI = URI.create(replicaLocation + partitionMexico);
    replicaCatalog
        .client()
        .add_partitions(
            Arrays
                .asList(newTablePartition(replicaTable, Arrays.asList("America", "Mexico"), partitionURI)));
  }
